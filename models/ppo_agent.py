...
class PPO:
    ...
    def clear_memory(self):
        if torch.cuda.is_available():
            ...
            self.reward_d_memory = torch.Tensor([]).cuda()
            self.reward_mi_memory = torch.Tensor([]).cuda()
            ...
            self.reward_d_memory = torch.Tensor([])
            self.reward_mi_memory = torch.Tensor([])
            self.reward_memory = torch.Tensor([])
            self.done_memory = torch.Tensor([])

    def update_memory(self, state, action, r_d, r_mi, done):
        self.state_memory = torch.cat((self.state_memory, state), dim=0)
        self.action_memory = torch.cat((self.action_memory, action), dim=0)
        self.reward_d_memory = torch.cat((self.reward_d_memory, r_d), dim=0)
        self.reward_mi_memory = torch.cat((self.reward_mi_memory, r_mi), dim=0)
    ...
    def reward_scaling(self, lambd=1):
        self.reward_d_memory = (self.reward_d_memory - self.reward_d_memory.min())/(self.reward_d_memory.max() - self.reward_d_memory.min())
        self.reward_mi_memory = (self.reward_mi_memory - self.reward_mi_memory.min())/(self.reward_mi_memory.max() - self.reward_mi_memory.min())
        self.reward_memory = self.reward_d_memory + lambd*self.reward_mi_memory
